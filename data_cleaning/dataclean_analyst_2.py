...
def working_program(stk):
    ...
    for year in year_list:
        ...
        ANALYST_t = math.log(ANALYST_t+1, math.e)

        # # 计算当年预测的乐观倾向，是FO > 0 占当年的比例
        # df_ana_stk_yr = df_ana_stk_yr.dropna(subset=['ForecastOptimism'])
        #
        # if (ANALYST_t == 0): OPTIMISM_t = None
        # else:
        #     opt_n = 0
        #     for id in df_analy_id:
        #         da = df_ana_stk_yr[df_ana_stk_yr['AnanmID'] == id]
        #         df_temp = da[da['ForecastOptimism'] > 0]
        #         # print(df_temp)
        #         if(len(df_temp) > 0): opt_n += 1
        #
        #     opt_d = len(df_analy_id)
        #     OPTIMISM_t = (opt_n / opt_d) if (opt_d != 0) else None
        #     # print(stk, year, opt_n, opt_d, ANALYST_t)

        # data_out.append([stk, year, ANALYST_t, OPTIMISM_t])
        data_out.append([stk, year, ANALYST_t])
...
with open("analyst_result.csv", 'w', encoding='utf-8') as rw:
    res_w = csv.writer(rw)
    res_w.writerow(["stock_code", "year", "Analyst"])
